class WatcherGroupsController < ApplicationController
  ...
  def create
    if params[:watcher_group].is_a?(Hash) && request.post?
      ...
      group_ids.each do |group_id|
        if !Watcher.where("watchable_type='#{@watched.class}' and watchable_id = #{@watched.id} and user_id = ?", group_id).exists?
          # insert directly into table to avoit user type checking
          quote_group_id = ActiveRecord::Base.connection.quote_string("#{group_id}")
          Watcher.connection.execute("INSERT INTO `#{Watcher.table_name}` (`user_id`, `watchable_id`, `watchable_type`) VALUES (#{quote_group_id}, #{@watched.id}, '#{@watched.class.name}')")
        end
      end
    end
  ...
  def autocomplete_for_group
    @groups = Group.active.like(params[:q]).limit(100)
    if @watched
...
private
  ...
  def permit_params
    params.permit(:q, :group_id, :object_type, :object_id, :project_id, watcher_group: [:group_ids, :group_id ])
  end
end
