...
class RedmineMailHandler
  ...
  def initialize
    ...
    optparse = OptionParser.new do |opts|
      ...
      opts.separator("")
      opts.separator("Ticket attributes control options:")
      ...
      opts.on(      "--private",              "create new ticket as private") {|v| self.issue_attributes['is_private'] = '1'}
      opts.on(      "--due-date DUE_DATE",    "due date for ticket") {|v| self.issue_attributes['due_date'] = v}
      opts.on(      "--assigned_to ASSIGNED_LOGIN", "assigned user login") {|v| self.issue_attributes['assigned_to'] = v}
      ...
      opts.separator("  rdm-helpdesk-mailhandler.rb --url http://redmine.domain.foo --key secret --project test-project\\")
      opts.separator("    --category foo \\")
      opts.separator("    --tracker bug \\")
      opts.separator("    --allow-override tracker,project")
